...
function buildBars(
  ...
  const adjustedWidth = frameWidth - interiorGap * 2;
  const adjustedHeight = frameHeight - interiorGap * 2;
  ...
  const addLines = (
    ...
    for (let i = 1; i <= count; i++) {
      ...
      const start = isHorizontal
        ? new THREE.Vector3(-adjustedWidth / 2, offset, 0.02)
        : new THREE.Vector3(offset, -adjustedHeight / 2, 0.02);
  ...
  const addDivisions = (
    ...
    for (let i = 1; i < divisions; i++) {
      const offset = -fullLength / 2 + (i * fullLength) / divisions;
      const geometry = isVertical
        ? new THREE.PlaneGeometry(barThickness, barLength)
        : new THREE.PlaneGeometry(barLength, barThickness);
  ...
  if (horGlazingBarsNumber > 0)
    addLines(horGlazingBarsNumber, true, adjustedHeight);

  if (verGlazingBarsNumber > 0)
    addLines(verGlazingBarsNumber, false, adjustedWidth);

  if (stileNb > 0)
    addDivisions(stileNb, true, frameThickness, adjustedHeight - 2 * frameThickness, adjustedWidth);

  if (railNb > 0)
    addDivisions(railNb, false, frameThickness, adjustedWidth - 2 * frameThickness, adjustedHeight);
}
